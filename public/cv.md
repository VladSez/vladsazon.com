---
type: Reference
title: Vlad Sazonau's CV / Resume
description: Resume of Vlad Sazonau, a product engineer with 8+ years of experience. Specialized in React, Next.js, TypeScript, Node.js, and full-stack development. Experienced with modern AI tools/Agentic development patterns. Featured EasyInvoicePDF project with 1k+ GitHub stars.
resource: https://vladsazon.com/vlad-sazon-cv.pdf
tags:
  [
    cv,
    resume,
    software-engineer,
    react,
    nextjs,
    typescript,
    full-stack,
    vlad sazonau,
  ]
timestamp: 2026-06-03T00:00:00Z
---

# Vlad Sazonau's CV / Resume

---

**Note:** The formatted PDF version remains canonical at https://vladsazon.com/vlad-sazon-cv.pdf

## About

Pragmatic, product-focused software engineer with 8+ years of experience building and shipping web products. Specialized in React, Next.js, TypeScript, and Node.js. Experienced in full-stack development, from UX decisions to production infrastructure. Proficient with modern AI tools and agentic development patterns.

For the past 3.5 years, I've been building software for the maritime industry. In my spare time, I've been working on [EasyInvoicePDF](https://easyinvoicepdf.com) - a free, open-source invoice generator.

## Contact

- Email: [smallredeagle@gmail.com](mailto:smallredeagle@gmail.com)
- Website: [vladsazon.com](http://vladsazon.com)
- GitHub: [github.com/VladSez](https://github.com/VladSez)
- LinkedIn: [linkedin.com/in/vlad-sazonau](https://www.linkedin.com/in/vlad-sazonau/)

## Key Skills

### Frontend

React, Next.js, TypeScript, Tailwind CSS, shadcn/ui, Radix UI/Base UI, Motion, Apollo Client, tRPC, Tanstack Router

### Backend

Node.js, Express/hono, Cloudflare Workers, GraphQL, REST APIs, PostgreSQL, Prisma/Drizzle ORM

### DevOps & Tools

Vercel, Docker, Git, CI/CD, Modern AI tools (Cursor, Claude Code/OpenCode)

### Design

UI/UX Design, Web Performance, Accessibility, Responsive Design, Web Vitals, Framer Motion, Figma

## Featured Project

### EasyInvoicePDF

Open-source invoice generator built with React, Next.js, and TypeScript. Features customizable templates, real-time preview, and PDF export.

**Metrics:**

- 1k+ GitHub stars
- 2k monthly visitors
- 2k+ total invoices generated

**Links:**

- [Live Demo](https://easyinvoicepdf.com)
- [GitHub Repository](https://github.com/VladSez/easy-invoice-pdf)

## Links

- [Download CV (PDF)](https://vladsazon.com/vlad-sazon-cv.pdf)
- [Projects](https://vladsazon.com/projects.md)
- [LinkedIn Profile](https://www.linkedin.com/in/vlad-sazonau/)
- [GitHub](https://github.com/VladSez)
- [X / Twitter](https://x.com/vladsazonau)
